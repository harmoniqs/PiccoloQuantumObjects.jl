...
varsys = VariationalQuantumSystem(H_drift, H_drives, H_vars)
...
!!! note "No drift constructor"
    ...
    ```julia
    varsys = VariationalQuantumSystem(H_drives, H_vars)
    ```
=#

# _Create a variational system with only drive and variational operators._
varsys_nodrift = VariationalQuantumSystem([PAULIS[:X], PAULIS[:Y]], [PAULIS[:Z]])
...
varsys_func = VariationalQuantumSystem(H_func, H_var_funcs, 2)
